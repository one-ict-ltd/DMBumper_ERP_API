...
namespace ONEERP.Areas.Hrm.Controllers
{
    ...
    public class BloodGroupController : Controller
    {
        ...
        public BloodGroupController(IUserInfoes userInfoes, IHrmMasterService employeeInfoService)
        {
        ...
        [HttpPost("setBloodGroup")]
        public async Task<IActionResult> SetBloodGroup([FromBody] BloodGroupViewModel model)
        {
            ...
            bool result = await hrmMasterService.SaveBloodGroup(user.employeeId.ToString(), model);
            if (result == true)
            {
        ...
        public async Task<IActionResult> GetBloodGroup(int bloodGroupId)
        {
            var uid = Request.Headers["auth_token"];
            if (uid.Count() == 0)
            {
            ...
            var datajson = await hrmMasterService.GetBloodGroupById(bloodGroupId);
            var jwt = await Tokens.getData(datajson.data, new JsonSerializerSettings { Formatting = Formatting.Indented });
            return new OkObjectResult(jwt);
        }

        [HttpPost("deleteBloodGroup")]
        public async Task<IActionResult> deleteBloodGroup([FromBody] BloodGroupViewModel model)
        {
            var uid = Request.Headers["auth_token"];
            if (uid.Count() == 0)
            {
            ...
            bool result = await hrmMasterService.DeleteBloodGroupById(user.employeeId.ToString(), (int)model.bloodGroupId);
            if (result == true)
            {
