...
namespace ONEERP.Areas.Hrm.Controllers
{
    ...
    public class ThanaController : Controller
    {
        ...
        public ThanaController(IUserInfoes userInfoes, IHrmMasterService employeeInfoService)
        {
        ...
        [HttpPost("setThanas")]
        public async Task<IActionResult> SetThanas([FromBody] ThanasViewModel model)
        {
            ...
            bool result = await hrmMasterService.SaveThanas(user.employeeId.ToString(), model);
            if (result == true)
            {
        ...
        [HttpGet("getThanas")]
        public async Task<IActionResult> GetThanas(int districtsId)
        {
            var uid = Request.Headers["auth_token"];
            if (uid.Count() == 0)
            {
            ...
            var datajson = await hrmMasterService.GetThanasById(districtsId);
            var jwt = await Tokens.getData(datajson.data, new JsonSerializerSettings { Formatting = Formatting.Indented });
            return new OkObjectResult(jwt);
        }

        [HttpPost("deleteThanas")]
        public async Task<IActionResult> deleteThanas([FromBody] ThanasViewModel model)
        {
            var uid = Request.Headers["auth_token"];
            if (uid.Count() == 0)
            {
            ...
            bool result = await hrmMasterService.DeleteThanasById(user.employeeId.ToString(), (int)model.thanasId);
            if (result == true)
            {
