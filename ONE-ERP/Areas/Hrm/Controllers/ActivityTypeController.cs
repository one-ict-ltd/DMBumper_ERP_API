...
namespace ONEERP.Areas.Hrm.Controllers
{
    ...
    public class ActivityTypeController : Controller
    {
        ...
        public ActivityTypeController(IUserInfoes userInfoes, IHrmMasterService employeeInfoService)
        {
        ...
        [HttpPost("setActivityType")]
        public async Task<IActionResult> SetActivityType([FromBody] ActivityTypeViewModel model)
        {
            ...
            bool result = await hrmMasterService.SaveActivityType(user.employeeId.ToString(), model);
            if (result == true)
            {
        ...
        public async Task<IActionResult> GetActivityType(int activityTypeId)
        {
            var uid = Request.Headers["auth_token"];
            if (uid.Count() == 0)
            {
            ...
            var datajson = await hrmMasterService.GetActivityTypeById(activityTypeId);
            var jwt = await Tokens.getData(datajson.data, new JsonSerializerSettings { Formatting = Formatting.Indented });
            return new OkObjectResult(jwt);
        }

        [HttpPost("deleteActivityType")]
        public async Task<IActionResult> deleteActivityType([FromBody] ActivityTypeViewModel model)
        {
            var uid = Request.Headers["auth_token"];
            if (uid.Count() == 0)
            {
            ...
            bool result = await hrmMasterService.DeleteActivityTypeById(user.employeeId.ToString(), (int)model.activityTypeId);
