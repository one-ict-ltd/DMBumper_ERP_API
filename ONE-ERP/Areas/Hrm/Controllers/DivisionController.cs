...
namespace ONEERP.Areas.Hrm.Controllers
{
    ...
    public class DivisionController : Controller
    {
        ...
        public DivisionController(IUserInfoes userInfoes, IHrmMasterService employeeInfoService)
        {
        ...
        public async Task<IActionResult> GetAddressType()
        {
            ...
            var datajson = await hrmMasterService.GetAddressType();
        ...
        [HttpPost("setDivision")]
        public async Task<IActionResult> SetDivision([FromBody] DivisionViewModel model)
        {
            var uid = Request.Headers["auth_token"];
            if (uid.Count() == 0)
            {
            ...
            bool result = await hrmMasterService.SaveDivision(user.employeeId.ToString(), model);
            if (result == true)
            {
        ...
        [HttpGet("getDivision")]
        public async Task<IActionResult> GetDivision(int countryId)
        {
            var uid = Request.Headers["auth_token"];
            if (uid.Count() == 0)
            {
            ...
            var datajson = await hrmMasterService.GetDivisionById(countryId);
            var jwt = await Tokens.getData(datajson.data, new JsonSerializerSettings { Formatting = Formatting.Indented });
            return new OkObjectResult(jwt);
        }

        [HttpPost("deleteDivision")]
        public async Task<IActionResult> deleteDivision([FromBody] DivisionViewModel model)
        {
            var uid = Request.Headers["auth_token"];
            if (uid.Count() == 0)
            {
            ...
            bool result = await hrmMasterService.DeleteDivisionById(user.employeeId.ToString(), (int)model.divisionsId);
            if (result == true)
            {
