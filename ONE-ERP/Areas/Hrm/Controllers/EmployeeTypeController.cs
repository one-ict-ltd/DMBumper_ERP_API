...
namespace ONEERP.Areas.Hrm.Controllers
{
    ...
    public class EmployeeTypeController : Controller
    {
        ...
        public EmployeeTypeController(IUserInfoes userInfoes, IHrmMasterService employeeInfoService)
        {
        ...
        [HttpPost("setEmployeeType")]
        public async Task<IActionResult> SetEmployeeType([FromBody] EmployeeTypeViewModel model)
        {
            ...
            bool result = await hrmMasterService.SaveEmployeeType(user.employeeId.ToString(), model);
            if (result == true)
            {
        ...
        public async Task<IActionResult> GetEmployeeType(int employeeTypeId)
        {
            ...
            var datajson = await hrmMasterService.GetEmployeeTypeById(employeeTypeId);
            var jwt = await Tokens.getData(datajson.data, new JsonSerializerSettings { Formatting = Formatting.Indented });
            return new OkObjectResult(jwt);
        }

        [HttpPost("deleteEmployeeType")]
        public async Task<IActionResult> deleteDesignation([FromBody] EmployeeTypeViewModel model)
        {
            var uid = Request.Headers["auth_token"];
            if (uid.Count() == 0)
            {
            ...
            bool result = await hrmMasterService.DeleteEmployeeTypeById(user.employeeId.ToString(), (int)model.employeeTypeId);
            if (result == true)
            {
